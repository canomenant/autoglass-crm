...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#fff;border-radius:12px;padding:28px">
  <tr><td>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="font-size:13px;margin:0 0 18px">${filasTrabajos}</table>` : ""}
  ...
  const text = [
    ...
    "",
    ...jobs.map((o) => `- ${o.workOrderNo || "—"} · ${o.workDate ? String(o.workDate).slice(0, 10) : "—"} · ${o.customerName || "—"}${o.vehicle ? ` (${o.vehicle})` : ""} · ${money(o.amount)}`),
    "",
