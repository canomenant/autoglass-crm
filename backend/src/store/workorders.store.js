...
// Toda columna del catálogo de la lista (frontend/src/lib/workOrdersTableColumns.js) se puede
// ordenar de mayor a menor y al revés, incluidas las que se agregan desde Configure View
// (Antonio, 7-sep-2026). Los accesores calcan getColumnValue; lo que depende de otra tabla
// (facturas, catálogo de técnicos) no entra.
const num = (v) => Number(v) || 0;
const ultimoPago = (w) => { const h = w.paymentHistory?.[w.paymentHistory.length - 1]; return h?.timestamp ? String(h.timestamp).slice(0, 10) : ""; };
const SORTABLE_FIELDS = {
  woNo: (w) => w.workOrderNo,
  quoteNo: (w) => w.quoteNo,
  status: (w) => w.status,
  type: (w) => w.workOrderType || "Personal",
  priority: (w) => w.priority,
  jobType: (w) => w.jobType,
  serviceType: (w) => w.jobType,
  createdDate: (w) => w.createdAt,
  lastUpdated: (w) => w.updatedAt,
  completionDate: (w) => (COMPLETED_STATUSES.includes(w.status) ? w.updatedAt : ""),
  assignedTech: (w) => w.tech,
  specialInstructions: (w) => w.specialInstructions,
  customerName: (w) => w.customerName,
  firstName: (w) => String(w.customerName || "").split(" ")[0],
  lastName: (w) => String(w.customerName || "").split(" ").slice(1).join(" "),
  phone: (w) => w.phone,
  mobile: (w) => w.mobile,
  email: (w) => w.email,
  address: (w) => w.address,
  serviceAddress: (w) => w.address,
  city: (w) => w.city,
  serviceCity: (w) => w.city,
  state: (w) => w.state || w.customerState,
  serviceState: (w) => w.state || w.customerState,
  zipCode: (w) => w.zipCode,
  serviceZipCode: (w) => w.zipCode,
  year: (w) => w.vehicle?.year,
  make: (w) => w.vehicle?.make,
  model: (w) => w.vehicle?.model,
  bodyStyle: (w) => w.vehicle?.bodyType,
  vin: (w) => w.vehicle?.vin,
  plate: (w) => w.vehicle?.plate,
  insuranceCompany: (w) => w.insuranceCompanyId,
  claimNumber: (w) => w.claimNumber,
  policyNumber: (w) => w.policyNumber,
  deductible: (w) => num(w.deductible),
  agentName: (w) => w.agentName,
  authorizationNumber: (w) => w.payment?.authorizationId,
  appointmentDate: (w) => w.appointmentDate,
  appointmentTime: (w) => w.appointmentTime,
  partNumber: (w) => w.partNumber,
  partDescription: (w) => w.nagsDescription || w.partDescriptions,
  glassType: (w) => w.glassType,
  priceTier: (w) => w.priceTier,
  calibrationRequired: (w) => (w.calibrationType ? 1 : 0),
  calibrationCost: (w) => num(w.calibrationCost),
  distributorName: (w) => w.distributor || w.distributorFromLines,
  poNumber: (w) => w.orderNumber,
  distributorCost: (w) => num(w.distributorCost ?? w.glassCost),
  assignmentDate: (w) => w.techAssignedAt,
  ...
  remainingBalance: (w) => Math.max(0, num(w.totalSale) - num(w.payment?.amount)),
  glassCost: (w) => num(w.glassCost),
  laborCost: (w) => num(w.laborCost),
  commission: (w) => num(w.commission),
  tax: (w) => num(w.totalSale) * (num(w.taxRate) / 100),
  discount: (w) => (w.discountType === "Percentage" ? (num(w.totalSale) * num(w.discountValue)) / 100 : num(w.discountValue)),
  upsell: (w) => num(w.upsell),
  totalCost: (w) => num(w.glassCost) + num(w.laborCost),
  totalSale: (w) => num(w.totalSale),
  grossProfit: (w) => num(w.totalSale) - num(w.glassCost) - num(w.laborCost) - num(w.commission),
  photosUploaded: (w) => (w.techPhotos || []).length,
};
