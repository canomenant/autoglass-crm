...
export default function SendLinkMenu({ label, phone, email, text, subject, url, onSent, disabled, className = "", labels = {} }) {
  const [open, setOpen] = useState(false);
  const [copied, setCopied] = useState(false);
  const ref = useRef(null);
  const L = { sms: "Text message (SMS)", whatsapp: "WhatsApp", email: "Email", copy: "Copy link", copied: "Link copied!", noPhone: "no phone", noEmail: "no email", ...labels };
