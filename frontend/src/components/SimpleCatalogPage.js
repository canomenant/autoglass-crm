...
export default function SimpleCatalogPage({ icon, translationKey, getItems, createItem, updateItem, deleteItem }) {
  const t = useTranslations(`settingsCatalog.${translationKey}`);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <SettingsIcon name={icon} className="w-6 h-6 text-gray-800" />
          <div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {items.length === 0 && !error && (
              <tr><td className="p-4 text-gray-500" colSpan={3}>{tc("noRecords")}</td></tr>
            )}
          </tbody>
        </table>
      </div>
