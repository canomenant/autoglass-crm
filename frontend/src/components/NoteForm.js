...
export default function NoteForm({ noteType, initialData, onSubmit, submitLabel }) {
  ...
  // Elegir la parte devuelta rellena lo que su nota de débito ya sabe — entidad, parte, monto y
  // razón — y deja solo la factura del crédito y la fecha para teclear (pedido de Antonio,
  // 30-ago-2026: "que se auto-rellenen los campos"). Si el distribuidor existe en el catálogo se
  // selecciona por id; si no, al menos viaja el nombre, que es lo que guarda el backend.
  function handleDebitNoteChange(id) {
    if (!id) { set("debitNoteId", ""); return; }
        ...
        {noteType === "CREDIT" && (
          <div className="mt-4 border border-green-200 dark:border-green-500/30 bg-green-50/60 dark:bg-green-500/5 rounded-lg p-3">
            ...
            <select
              ...
              onChange={(e) => handleDebitNoteChange(e.target.value)}
              className="w-full border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-shadow text-sm"
            >
