...
import { getCreditNotes, getCreditNotesDashboard, applyCreditNote, voidCreditNote } from "@/lib/api";
...
export default function CreditNotesPage() {
  ...
  function load() {
    getCreditNotes(filters).then(setNotes).catch((e) => setError(e.message));
    getCreditNotesDashboard().then(setKpis).catch(() => {});
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between my-4">
        <div>
          <h1 className="text-2xl font-semibold dark:text-gray-100 tracking-tight">{t("creditNotesTitle")}</h1>
          <p className="text-sm text-gray-500">{t("creditNotesSubtitle")}</p>
        </div>
        <div className="flex gap-2">
          <button onClick={handleExportCsv} className="border rounded px-4 py-2 text-sm">{tp("exportCsv")}</button>
          <Link href="/dashboard/payments/credit-notes/create" className="bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors px-4 py-2 text-sm">
            {t("newCreditNote")}
          </Link>
        </div>
      </div>
              ...
              <tr key={n.id} className="border-b last:border-0 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/60 transition-colors">
                ...
                <td className="p-3">
                  <div className="flex items-center justify-end gap-3">
                    ...
                    <Link href={`/dashboard/payments/credit-notes/${n.id}`} className="text-gray-600 text-xs">{tc("viewEdit")}</Link>
                  </div>
                </td>
              </tr>
            ))}
