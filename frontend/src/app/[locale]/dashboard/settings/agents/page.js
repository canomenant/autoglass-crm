...
export default function AgentsListPage() {
  ...
  function load() {
    getAgents().then(setItems).catch((e) => setError(e.message));
  }
  ...
  async function toggleStatus(item) {
    try {
      await updateAgent(item.id, { status: item.status === "Active" ? "Inactive" : "Active" });
      load();
    } catch (e) {
      setError(e.message);
    }
              ...
              <tr key={item.id} className="border-b last:border-0 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/60 transition-colors">
                ...
                <td className="p-4">
                  <div className="flex justify-end gap-3">
                    <Link href={`/dashboard/settings/agents/${item.id}`} className="text-blue-600 dark:text-blue-400 hover:underline">{tc("viewEdit")}</Link>
