...
const rowColors = ["text-blue-600", "text-orange-600", "text-blue-600"];
...
export default function PriceTierPage() {
  const t = useTranslations("settingsCatalog.priceTier");
  ...
  function load() {
    getPriceTiers().then(setItems).catch((e) => setError(e.message));
  }
  ...
  function openEdit(item) {
    setEditing(item);
    setForm({ name: item.name, amount: item.amount, description: item.description || "" });
    setModalOpen(true);
  }
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      if (editing) {
        await updatePriceTier(editing.id, payload);
      } else {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <SettingsIcon name="layers" className="w-6 h-6 text-gray-800" />
          <div>
      ...
      <div className="bg-white dark:bg-gray-900 dark:border dark:border-gray-800 rounded-xl shadow-sm overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left border-b dark:border-gray-800 text-xs text-blue-600 uppercase tracking-wide">
              <th className="p-4">#</th>
              <th className="p-4">{t("column")}</th>
              <th className="p-4">{tc("amount")}</th>
            ...
            {items.map((item, i) => {
              ...
              return (
                <tr key={item.id} className="border-b last:border-0 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/60 transition-colors">
                  <td className={`p-4 font-medium ${color}`}>{i + 1}</td>
                  <td className={`p-4 ${color}`}>{item.name}</td>
                  <td className="p-4">{item.amount > 0 ? `$${Number(item.amount).toFixed(2)}` : "—"}</td>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <form onSubmit={handleSave} className="bg-white dark:bg-gray-800 dark:border dark:border-gray-700 rounded-xl shadow-xl p-6 w-full max-w-sm space-y-4">
            ...
            <div>
              <label className="block text-sm mb-1 text-gray-600 dark:text-gray-300">{t("description")}</label>
              <input
