...
import {
  getCalibrationTypes,
  createCalibrationType,
  updateCalibrationType,
  deleteCalibrationType,
} from "@/lib/api";

const badgeColors = ["text-teal-600", "text-blue-600", "text-purple-600", "text-orange-600"];
...
export default function CalibrationTypePage() {
  const t = useTranslations("settingsCatalog.calibrationType");
  ...
  function load() {
    getCalibrationTypes().then(setItems).catch((e) => setError(e.message));
  }
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      if (editing) {
        await updateCalibrationType(editing.id, payload);
      } else {
        await createCalibrationType(payload);
      }
      setModalOpen(false);
      load();
    } catch (e) {
  ...
  async function handleDelete(id) {
    ...
    try {
      await deleteCalibrationType(id);
      load();
    } catch (e) {
      setError(e.message);
    }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <SettingsIcon name="target" className="w-6 h-6 text-gray-800" />
          <div>
          ...
          <tbody>
            {items.map((item, i) => (
              <tr key={item.id} className="border-b last:border-0 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/60 transition-colors">
                <td className={`p-4 font-medium ${badgeColors[i % badgeColors.length]}`}>{i + 1}</td>
